  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Panel title="Catálogo de productos" description="Visualizá inventario, editá datos, filtrá por categoría y eliminá productos obsoletos." action={<div className="flex flex-wrap gap-2"><SummaryBadge label="Productos visibles" value={filteredItems.length} /><SummaryBadge label="Cobertura sana" value={`${stockCoverage}%`} /></div>}>
          {loading ? <EmptyState>Cargando inventario...</EmptyState> : filteredItems.length === 0 ? <EmptyState>No hay productos para ese filtro. Probá otra categoría o limpiá la búsqueda.</EmptyState> : <InventoryTable items={filteredItems} onEdit={startEditing} onDelete={handleDelete} formatMoney={formatMoney} />}
        </Panel>
        <div className="space-y-6">
