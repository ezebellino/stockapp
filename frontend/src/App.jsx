          ...
          <div className="grid gap-6 px-6 py-7 lg:grid-cols-[1.4fr_1fr]">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-3xl text-sm text-slate-300 sm:text-base">Ahora el flujo de escaner mantiene foco, confirma cada lectura y bloquea dobles ingresos accidentales.</p>
              </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
          <Panel title="Ingreso por escaner" description="Autoenfoque constante, enter del lector y proteccion contra doble lectura." action={<button type="button" onClick={focusScanner} className="rounded-full border border-sky-400/40 bg-sky-500/10 px-4 py-2 text-sm font-medium text-sky-200 transition hover:bg-sky-500/20">Enfocar</button>}>
            <div className="mb-4 grid gap-3 sm:grid-cols-[1fr_auto]">
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-xs uppercase tracking-[0.2em] text-slate-300">
                Enter del lector: envio inmediato
              </div>
            </div>
            <form className="space-y-4" onSubmit={submitScan}>
              <InputField
                ref={scanInputRef}
                label="Codigo escaneado"
                name="scanCode"
                value={scanCode}
...
function ScannerStatus({ state }) {
  const states = {
    ready: { label: "Lector listo", className: "border-emerald-400/30 bg-emerald-400/10 text-emerald-100" },
    processing: { label: "Procesando lectura", className: "border-sky-400/30 bg-sky-400/10 text-sky-100" },
    success: { label: "Lectura confirmada", className: "border-emerald-400/30 bg-emerald-400/10 text-emerald-100" },
    warning: { label: "Codigo nuevo", className: "border-amber-400/30 bg-amber-400/10 text-amber-100" },
    error: { label: "Revisar lectura", className: "border-rose-400/30 bg-rose-400/10 text-rose-100" },
    blocked: { label: "Doble lectura bloqueada", className: "border-orange-400/30 bg-orange-400/10 text-orange-100" },
  };
  const current = states[state] ?? states.ready;
  return <div className={`rounded-2xl border px-4 py-3 text-sm font-medium ${current.className}`}>{current.label}</div>;
}
